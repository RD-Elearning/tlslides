export * from './alignShapes'
export * from './changePage'
export * from './createPage'
export * from './createShapes'
export * from './deletePage'
export * from './deleteShapes'
export * from './distributeShapes'
export * from './duplicatePage'
export * from './duplicateShapes'
export * from './flipShapes'
export * from './groupShapes'
export * from './insertContent'
export * from './movePage'
export * from './moveShapesToPage'
export * from './moveShapeToIndex'
export * from './reorderShapes'
export * from './renamePage'
export * from './resetBounds'
export * from './setPageSize'
export * from './setPageBackground'
export * from './setPageNotes'
export * from './setPageSkipInPresentation'
export * from './setDeckTheme'
export * from './addSlideFromTemplate'
export * from './rotateShapes'
export * from './stretchShapes'
export * from './styleShapes'
export * from './toggleShapesDecoration'
export * from './toggleShapesProp'
export * from './translateShapes'
export * from './ungroupShapes'
export * from './updateShapes'
export * from './setShapesProps'
